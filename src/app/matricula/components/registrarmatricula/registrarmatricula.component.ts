...
import { DocumentoEntidad, CategoriaDocumento, SubCategoria } from '../../interfaces/DTOEntidad';
import {
...
export class RegistrarMatriculaComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.route.queryParams.subscribe(params => {
        ...
        initialObservables.datosMatriculaContinuar = this.matriculaService.obtenerMatricula(this.idMatriculaContinuar).pipe(
          switchMap(matricula => {
            if (!matricula || !matricula.idapoderado || !matricula.idalumno) {
              this.notificationService.showNotification('No se pudo cargar la información completa de la matrícula', 'error');
              return throwError(() => new Error('Datos de matrícula incompletos o no encontrados. '));
            }
      ...
      if (!this.esContinuacion || (this.matriculaOriginal && !this.matriculaOriginal.seccion)) {
         initialObservables.assignedSection = this.matriculaService.asignarSeccion(this.nivel, this.grado).pipe(
            catchError(err => {
      ...
      forkJoin(initialObservables).pipe(
        ...
        next: (results: any) => {
          if (this.esContinuacion && results.datosMatriculaContinuar && results.datosMatriculaContinuar.matricula && results.datosMatriculaContinuar.matricula.seccion) {
            this.seccion = typeof results.datosMatriculaContinuar.matricula.seccion === 'string'
              ? results.datosMatriculaContinuar.matricula.seccion
              : String(results.datosMatriculaContinuar.matricula.seccion || 'ERROR');
        ...
        error: (err) => {
          this.notificationService.showNotification('Error general durante la inicialización: ' + (err.message || 'Error desconocido. Verifique duplicados en documento del alumno.'), 'error');
          this.isLoading = false;
  ...
  poblarFormularioParaContinuacion(matricula: Matricula, apoderado: Apoderado, alumno: Alumno): void {
    ...
    this.notificationService.showNotification('Datos de matrícula cargados.', 'info');
  }
  loadEntityDocuments(): Observable<void> {
  return this.entidadService.obtenerEntidadPorUsuario(this.usuario.idusuario).pipe(
    tap(response => {
      if (response && response.documentos) {
        this.entidadDocumentos = response.documentos;
        this.allPotentialDocuments = [];
        this.necesariosMap = {};
        this.intercambioMap = {};
        this.discapacidadMap = {};

        if (this.entidadDocumentos.categorias) {
          // Buscar categoría "Necesarios" y poblar necesariosMap + allPotentialDocuments
          const catNecesarios = this.entidadDocumentos.categorias.find(c => c.nombre?.toLowerCase() === 'necesarios');
          if (catNecesarios?.documentos && catNecesarios.documentos.length > 0) {
            catNecesarios.documentos.forEach(doc => {
              if (doc && doc.trim() !== '') {
                this.allPotentialDocuments.push(doc);
                this.necesariosMap[doc] = true;
              }
            });
          }

          // Buscar categoría "Adicionales" y poblar sub-maps + allPotentialDocuments
          const catAdicionales = this.entidadDocumentos.categorias.find(c => c.nombre?.toLowerCase() === 'adicionales');
          if (catAdicionales?.subCategorias && catAdicionales.subCategorias.length > 0) {
            catAdicionales.subCategorias.forEach(sub => {
              if (sub.nombre && sub.documentos && sub.documentos.length > 0) {
                const subNameLower = sub.nombre.toLowerCase();
                sub.documentos.forEach(doc => {
                  if (doc && doc.trim() !== '' && !this.allPotentialDocuments.includes(doc)) {
                    this.allPotentialDocuments.push(doc);
                  }
                });

                // Poblar maps específicos por nombre de sub-categoría
                if (subNameLower === 'intercambio') {
                  Object.keys(sub.documentos.reduce((acc: any, doc) => ({ ...acc, [doc]: true }), {})).forEach(doc => {
                    this.intercambioMap[doc] = true;
                  });
                } else if (subNameLower === 'discapacidad') {
                  Object.keys(sub.documentos.reduce((acc: any, doc) => ({ ...acc, [doc]: true }), {})).forEach(doc => {
                    this.discapacidadMap[doc] = true;
                  });
                }
              }
            });
  ...
  areAllRequiredDocumentsPresented(): boolean {
      let allRequiredPresented = true;
      const documentsArray = this.documentsPresentedControls;
      this.allPotentialDocuments.forEach((docName, index) => {
          if (this.necesariosMap[docName] && !documentsArray.at(index)?.value) {
              allRequiredPresented = false;
          }
      });
      return allRequiredPresented;
  }
  ...
  buscarApoderado(): void {
    ...
    this.apoderadoService.buscarPorNumeroDocumento(idtipodoc, numeroDocumento).pipe(
      ...
      next: (apoderado) => {
        ...
        this.mostrarFormularioApoderado = true;
        this.currentView = 'apoderado';
        this.notificationService.showNotification('Apoderado encontrado.', 'success');
  ...
  validarApoderado(): void {
    ...
    if (apoderadoGroup?.valid && relacionControl?.valid) {
      ...
      this.mostrarSeccionDocumentos = false;
      this.notificationService.showNotification('Datos del apoderado correctos.', 'info');
  ...
  validarAlumno(): void {
      ...
      if (alumnoGroup?.valid) {
          ...
          this.notificationService.showNotification('Datos del alumno correctos.', 'info');
  ...
  private processMatricula(estadoMatriculaDeseado: 'COMPLETADO' | 'EN PROCESO'): void {
    ...
    if (estadoMatriculaDeseado === 'COMPLETADO') {
        if (!this.areAllRequiredDocumentsPresented()) {
            this.currentView = 'documents';
            this.mostrarFormularioApoderado = true;
            this.mostrarFormularioAlumno = true;
            this.mostrarSeccionDocumentos = true;
            this.notificationService.showNotification('No se puede finalizar la matrícula hasta que confirme todos los documentos requeridos. Revise la sección de documentos.', 'error');
            setTimeout(() => document.getElementById('documents-fieldset')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 50);
            return;
        }
        ...
        if (this.formMatricula.invalid) {
         ...
         if (!this.esContinuacion && (!hasApoderadoData || !hasAlumnoData)) {
             this.notificationService.showNotification('Debe ingresar datos del apoderado y del alumno para guardar el trámite en proceso.', 'error');
             return;
         }
    }
    if (!this.seccion || this.seccion === 'SIN VACANTE' || this.seccion === 'ERROR') {
        this.notificationService.showNotification(`No se puede ${estadoMatriculaDeseado === 'COMPLETADO' ? 'registrar' : 'pausar'}: sección inválida (${this.seccion}) o sin vacantes. El documento del alumno no puede procesarse.`, 'error');
        return;
    }
    ...
    if (apoderadoData.idapoderado) {
        if (apoderadoFormGroup.dirty || estadoMatriculaDeseado === 'EN PROCESO') {
            apoderadoObservable = this.apoderadoService.editarApoderado(apoderadoData.idapoderado, apoderadoData).pipe(
                ...
                catchError(err => {
                  if (err.status === 409 || (err.error?.message && (err.error.message.toLowerCase().includes('duplicado') || err.error.message.toLowerCase().includes('ya existe')))) {
                    this.notificationService.showNotification('El documento del apoderado ya está registrado en el sistema.', 'error');
                  }
    ...
    apoderadoObservable.pipe(
            ...
            if (alumnoData.idalumno) {
                if (alumnoFormGroup.dirty || estadoMatriculaDeseado === 'EN PROCESO') {
                    alumnoObservable = this.alumnoService.editarAlumno(alumnoData.idalumno, alumnoData).pipe(
                         ...
                         catchError(err => {
                           if (err.status === 409 || (err.error?.message && (err.error.message.toLowerCase().includes('duplicado') || err.error.message.toLowerCase().includes('ya existe')))) {
                             this.notificationService.showNotification('El documento del alumno ya está registrado en el sistema.', 'error');
                           }
                           return throwError(() => err);
                         })
                    );
        ...
        switchMap(({ idApoderado, idAlumno }) => {
            ...
            if (estadoMatriculaDeseado === 'EN PROCESO' && !idApoderado && !idAlumno && !this.esContinuacion && !formData.relacionEstudiante && presentedDocumentsList.length === 0) {
                this.notificationService.showNotification('No hay suficientes datos para guardar en proceso.', 'error');
                return of(null);
            }
            ...
            if (err instanceof HttpErrorResponse) {
                 try {
                     ...
                     if (errorBody && errorBody.message) {
                       if (errorBody.message.toLowerCase().includes('duplicado') || errorBody.message.toLowerCase().includes('ya existe')) {
                         errorMsg = 'Error de duplicado: El documento del alumno ya está registrado en el sistema.';
                       } else {
                         errorMsg = errorBody.message + '. Verifique duplicados en documento del alumno.';
                       }
                     } else {
                       errorMsg = `Error del servidor (${err.status}): ${err.statusText || 'Mensaje desconocido'}. Posible duplicado en documento del alumno.`;
                     }
                 } catch (e) {
                     errorMsg = `Error de comunicación con el servidor (${err.status}). Verifique duplicados en documento del alumno.`;
                 }
            } else if (err instanceof Error) {
                 errorMsg = err.message + '. Verifique duplicados en documento del alumno.';
            }
